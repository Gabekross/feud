-- Keep the multi-session board usable when prior schema changes were applied manually.
...
begin
  if not exists (
    ...
    create policy "Allow anonymous session answer state reads"
      on public.session_answer_states for select using (true);
  end if;
  ...
  if not exists (
    ...
    create policy "Allow anonymous session answer state inserts"
      on public.session_answer_states for insert with check (true);
  end if;
  ...
  if not exists (
    ...
    create policy "Allow anonymous session answer state updates"
      on public.session_answer_states for update using (true) with check (true);
  end if;
end $$;
